...
const contextBootstrapPacks = ['planning', 'ai', 'agents', 'mcp', 'research'];
const contextBootstrapSkills = [
  'using-agent-skills',
  'context-engineering',
...
Usage:
  agent-skills list [--pack all|core,planning,ai,code,flutter,kotlin,javascript,design,git,agents,mcp,research,security,devops,data,ml,creative] [--source github:user/repo|https://github.com/user/repo] [--version v0.6.0]
  agent-skills recommend-flutter-stack [--size small|medium|large] [--flow simple|standard|event-heavy] [--async simple|complex]
  agent-skills install --project <path> [--packs core,planning,ai,code,security,devops] [--source github:user/repo|https://github.com/user/repo] [--targets codex,claude,gemini] [--version v0.6.0] [--skills all|name,name]
  agent-skills update --project <path> --version <vX.Y.Z>
  agent-skills doctor --project <path> [--packs flutter]
  agent-skills mcp [--source github:user/repo|https://github.com/user/repo] [--version v0.6.0]
...
export async function main(argv) {
  ...
  if (command === 'list') {
    printJson(await listSkills({ sourceRepo: args.source || 'local', version: args.version || 'v0.6.0', packs: args.pack || args.packs || 'all' }));
    return;
  }
  ...
  if (command === 'install') {
    ...
    const hasPacks = Boolean(args.packs);
    const hasSkills = Boolean(args.skills);
  ...
  if (command === 'mcp') {
    await runMcpServer({
      listSkills: (toolArgs = {}) => listSkills({ sourceRepo: toolArgs.sourceRepo || args.source || 'local', version: toolArgs.version || args.version || 'v0.6.0', packs: toolArgs.packs || 'all' }),
      loadSkillIndex,
      recommendFlutterStack: recommendStack,
      installSkills: (toolArgs) => {
        const hasPacks = Boolean(toolArgs?.packs?.length);
        const hasSkills = Boolean(toolArgs?.requestedSkills?.length);
        return installSkills({
          ...
          version: toolArgs.version || args.version || 'v0.6.0'
        });
      },
